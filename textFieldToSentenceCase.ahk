...
;============================================
	...
	ConvertToSentenceCase()
	{
		;enable or disable troubleshooting messages
			...
			sleepStd := 125		;ms for script to pause at several moments in order to prevent errors
			...
			Clipboard := RegExReplace(Clipboard, "(((^|([.!?:—–]+\s+))[a-z])| i | i')", "$U1")		
				...
				sleep, sleepStd
					...
					;the . allows continuation of the previous line
						;convert to ALL CAPS	
						; Clipboard := RegExReplace(Clipboard, "i)((MOOC)|(\bSTEM)|(\b\dD\b)|(\bBME\b)|"
											; . "(PBL\b)|(P12)|(P-12)|(K12)|(K-12)|(IEEE)|(\bENE\b)|"
											; . "(\bR\b)|(\bNSF\b)|(\bEPICS\b)|(\bABET\b))", "$U1")
						Clipboard := RegExReplace(Clipboard, "i)((MOOC)|(\bSTEM)|(\b\dD\b)|(\bBME\b)|(\bIQ\b))", "$U1")
						Clipboard := RegExReplace(Clipboard, "i)((PBL\b)|(P12)|(P-12)|(K12)|(K-12)|(IEEE)|(\bENE\b))", "$U1")
						Clipboard := RegExReplace(Clipboard, "i)((\bR\b)|(\bNSF\b)|(\bEPICS\b)|(\bABET\b))", "$U1")
